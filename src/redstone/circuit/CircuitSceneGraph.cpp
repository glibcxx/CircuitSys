                    ...
                    std::queue<CircuitTrackingInfo> &queue)
{
    ...
    newInfo.mCurrent.set(newComponent, newPos, face, newComponent->getBaseType());
                    ...
                    bool                             goingDown)
{
    ...
    BaseCircuitComponent *Component = graph.getComponent(targetPos, ClassInstance::TypeID);
                                          ...
                                          std::queue<CircuitTrackingInfo> &positions)
{
    ...
    if (typeID == CS_Transporter)
    {
        ...
        BaseCircuitComponent *up = graph.getComponent(otherPos, CS_PoweredBlock);
        if (!up || up->canAllowPowerUp())
        {
            for (auto facing = 2; facing <= 5; facing++)
            {
                checkComponent<TransporterComponent>(graph, powerAssociationMap, facing, otherPos, info, positions, false);
            }
        }
        otherPos = targetPos + Facing::DIRECTION[0];
        BaseCircuitComponent *down = graph.getComponent(otherPos, CS_PoweredBlock);
        if (!down || !down->canAllowPowerUp())
        {
            for (auto facing = 2; facing <= 5; facing++)
            {
                checkComponent<TransporterComponent>(graph, powerAssociationMap, facing, otherPos, info, positions, true);
            }
        }
    }
    else if (typeID != CS_Producer || currentComponent->getDirection() != Facing::DOWN)
    {
        if (currentComponent->getInstanceType() == CS_BaseRail)
        {
            BlockPos otherPos = targetPos + Facing::DIRECTION[1];
            for (auto facing = 2; facing <= 5; facing++)
            {
                checkComponent<BaseRailTransporter>(graph, powerAssociationMap, facing, otherPos, info, positions, false);
            }
            otherPos = targetPos + Facing::DIRECTION[0];
            for (auto facing = 2; facing <= 5; facing++)
            {
                checkComponent<BaseRailTransporter>(graph, powerAssociationMap, facing, otherPos, info, positions, true);
            }
        }
    }
    else
    {
        ...
        PoweredBlockComponent *topComp = static_cast<PoweredBlockComponent *>(graph.getComponent(topPos, CS_PoweredBlock));
        PoweredBlockComponent *downComp = static_cast<PoweredBlockComponent *>(graph.getComponent(downPos, CS_PoweredBlock));
        if ((topComp || downComp)
            ...
            && (!topComp || topComp->isAllowedAsPowerSource()))
        {
                ...
                && (!topComp || topComp->canAllowPowerUp() || topComp->isPromotedToProducer()))
            {
                for (auto facing = 2; facing <= 5; facing++)
                {
                    checkComponent<TransporterComponent>(graph, powerAssociationMap, facing, downPos, info, positions, true);
                }
            }
        }
    }
...
void CircuitSceneGraph::add(const BlockPos &pos, std::unique_ptr<BaseCircuitComponent> component)
{
    ...
    if (iter != this->mPendingAdds.end())
    {
        ...
        if (pending.mComponent->getBaseType() == CS_PoweredBlock)
        {
        ...
        if (component->getBaseType() == CS_PoweredBlock)
        {
 ...
 * @return 返回原该元件的指针
 */
BaseCircuitComponent *CircuitSceneGraph::getComponent(const BlockPos &pos, uint64_t typeID)
{
    auto find = this->mAllComponents.find(pos);
    if (find != mAllComponents.end())
    {
        ...
        if (!systemComponent->isRemoved())
        {
            uint64_t idBase = systemComponent->getBaseType();
            uint64_t id = systemComponent->getInstanceType();
            uint64_t idComponent = typeID;
...
BaseCircuitComponent *CircuitSceneGraph::getFromPendingAdd(const BlockPos &pos, uint64_t typeID)
{
...
void CircuitSceneGraph::scheduleRelationshipUpdate(const BlockPos &pos, BaseCircuitComponent *component)
{
    ...
    if (typeID == CS_Capacitor || typeID == CS_Producer)
    {
...
void CircuitSceneGraph::processPendingAdds()
{
    for (auto iter = this->mPendingAdds.begin(); iter != mPendingAdds.end(); iter = this->mPendingAdds.erase(iter))
    {
        ...
        {
            if (componentGroupIter->second->getBaseType() == CS_PoweredBlock)
            {
                updatedType = component->getBaseType() != CS_PoweredBlock;
            }
        }
        if (componentGroupIter != mAllComponents.end() ? updatedType : true)
        {
            ...
            if (typeID == CS_Producer)
            {
                this->scheduleRelationshipUpdate(pos, component);
            }
            else if (typeID == CS_Capacitor)
            {
                this->scheduleRelationshipUpdate(pos, component);
                for (int corner1 = -1; corner1 <= 1; corner1 += 2)
                {
                    for (int corner2 = -1; corner2 <= 1; corner2 += 2)
                    {
                        ...
                        if (possibleComponent && possibleComponent->getBaseType() == CS_Capacitor)
                        {
            ...
            for (int iLevel = -1; iLevel <= 1; iLevel++)
            {
                for (int face = 0; face < 6; face++)
                {
                    ...
                    {
                        uint64_t typeID = possibleComponent->getBaseType();
                        if (typeID == CS_Producer || typeID == CS_Capacitor)
                        {
                            this->scheduleRelationshipUpdate(otherPos, possibleComponent);
                        }
                        else
...
void CircuitSceneGraph::findRelationships(const BlockPos &pos, BaseCircuitComponent *producerTarget, BlockSource *region)
{
    ...
    do
    {
        ...
        if (currentComponent)
        {
            ...
            if (dampening <= 15)
            {
                if (currentComponent->getBaseType() != CS_PoweredBlock)
                {
